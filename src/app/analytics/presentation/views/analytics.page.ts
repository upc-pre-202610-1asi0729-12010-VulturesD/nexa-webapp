...
import { AnalyticsStore } from '@app/analytics/application/analytics.store';
    ...
    <div class="page">
      ...
      @if (loading()) {
        ...
        <div class="banner banner-info">
          ...
          <div>{{ 'reports.periodBanner' | t }}</div>
        </div>
      }
    </div>
  `
...
export class AnalyticsPage {
  ...
  readonly activeDispatches = computed(() => this.dispatches().filter((d) => d.status === 'in_route').length);
  readonly pendingEvidence = computed(() => this.dispatches().filter((d) => d.status === 'delivered').length);
  ...
  constructor() {
    this.analytics.snapshot().subscribe({
