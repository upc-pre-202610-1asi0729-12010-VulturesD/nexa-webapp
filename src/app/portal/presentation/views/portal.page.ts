        ...
        <div class="flow-grid-12">
          ...
          <aside class="flow-panel span-7">
            ...
            <div class="flow-panel-pad quick-action-grid">
              ...
              <a routerLink="/portal/purchase-orders" class="secondary-btn">
...
export class PortalPage {
  ...
  activeOrders(): number {
    return this.snapshot()?.dashboardSummary?.activeOrdersCount
      ?? this.orders().filter((order) => !/delivered|completed|cancel/i.test(order.status)).length;
  }
  ...
  creditAvailable(): number {
    const summary = this.snapshot()?.dashboardSummary?.creditSummary;
    if (summary) return Number(summary.availableCredit || 0);
  ...
  requestsDesc(): string {
    return this.i18n.t('portal.homePanel.requestsDesc', {
      count: this.snapshot()?.dashboardSummary?.activePurchaseRequestsCount ?? this.requests().length,
    });
  }

  ordersDesc(): string {
    return this.i18n.t('portal.homePanel.ordersDesc', { count: this.activeOrders() });
  }

  creditDesc(): string {
    return this.i18n.t('portal.homePanel.creditDesc', {
      available: this.formatMoney(this.creditAvailable()),
      limit: this.formatMoney(
        this.snapshot()?.dashboardSummary?.creditSummary?.creditLimit ?? this.client()?.creditLimit ?? 0,
      ),
    });
  }

  pendingDocuments(): number {
    return this.snapshot()?.dashboardSummary?.pendingDocumentsCount
      ?? this.snapshot()?.documents.filter((document) => /pending|review/i.test(document.status)).length
      ?? 0;
  }
