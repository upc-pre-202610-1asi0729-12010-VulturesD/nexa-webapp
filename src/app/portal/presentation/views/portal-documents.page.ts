                ...
                <tbody>
                  @for (document of visibleDocuments(data); track document.id) {
                    <tr>
                      ...
                      <td>
                        <div class="flow-row">
                          <button type="button" class="secondary-btn btn-sm" [disabled]="!document.visibleToBuyer || downloading() === document.id" (click)="download(document)">{{ 'portal.documents.download' | t }}</button>
                          <a [routerLink]="['/portal/purchase-orders', document.orderId, 'tracking']" class="btn btn-ghost btn-sm">{{ 'portal.documents.tracking' | t }}</a>
                        </div>
                      </td>
                    </tr>
                  }
...
export class PortalDocumentsPage {
  ...
  download(document: BuyerDocument): void {
    this.downloading.set(document.id);
    this.store.downloadDocument(document.id).subscribe({
      next: (blob) => {
        const url = URL.createObjectURL(blob);
        const anchor = window.document.createElement('a');
        anchor.href = url;
        anchor.download = document.fileName || `${document.type}.bin`;
        anchor.click();
