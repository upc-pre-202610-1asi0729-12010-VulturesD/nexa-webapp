...
@Component({
    ...
    <div class="page">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
        <button class="btn btn-ghost btn-sm" (click)="router.navigate(['/orders'])"><i class="pi pi-arrow-left"></i> {{ t('nav.orders') }}</button>
        <div style="flex:1">
          <div class="page-title">{{ t('orders.new.title') }}</div>
          <div class="page-subtitle">{{ t('orders.new.subtitle') }}</div>
        </div>
      </div>
      ...
      @if (loading()) {
        ...
        @if (step() === 1) {
          <div>
            <div class="card-title" style="margin-bottom:12px">{{ t('orders.new.selectClient') }}</div>
                  ...
                  <button class="card card-pad client-select-card" [class.selected]="selectedClient()?.id === c.id" (click)="pickClient(c)">
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">
                      ...
                      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px">
                        <span [class]="'badge ' + (c.status === 'active' ? 'badge-green' : 'badge-orange')">{{ c.status === 'active' ? t('orders.new.clientActive') : t('orders.new.clientObserved') }}</span>
              ...
              <div style="position:sticky;top:24px">
                @if (!selectedClient()) {
                  <div class="card card-pad" style="text-align:center;color:#9CA3AF">
                    ...
                    <div style="font-size:13px">{{ t('orders.new.noClientHelp') }}</div>
                  </div>
                } @else {
                  <div class="card card-pad" style="margin-bottom:12px">
                    <div style="font-size:10px;font-weight:700;color:#2563EB;text-transform:uppercase;letter-spacing:.05em;margin-bottom:12px;display:flex;align-items:center;gap:5px">
                      <i class="pi pi-file-edit"></i> {{ t('orders.new.commercialConditions') }}
                    </div>
                    <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:8px">
                      <span style="color:#6B7280">{{ t('orders.new.paymentCondition') }}</span>
                      <span style="font-weight:600">{{ selectedClient()?.condition }}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:8px">
                      <span style="color:#6B7280">{{ t('orders.new.clientType') }}</span>
                      <span style="font-weight:600">{{ selectedClient()?.type }}</span>
                    </div>
                    <div class="divider" style="margin:10px 0"></div>
                    @if (selectedClient()?.creditLimit) {
                      <div style="font-size:10px;font-weight:600;color:#6B7280;text-transform:uppercase;margin-bottom:6px">{{ t('orders.new.credit') }}</div>
                      <div style="display:flex;justify-content:space-between;font-size:11px;color:#6B7280;margin-bottom:4px">
                        <span>{{ t('orders.new.creditUsed') }}: S/ {{ selectedClient()?.creditUsed || 0 | number:'1.0-0' }}</span>
                        <span>{{ t('orders.new.creditLimit') }}: S/ {{ selectedClient()?.creditLimit || 0 | number:'1.0-0' }}</span>
                      </div>
                      ...
                      @if (isCreditBlocked()) {
                        <div class="banner banner-danger" style="margin-top:8px"><i class="pi pi-times-circle"></i><div>{{ t('orders.new.creditBlocked') }}</div></div>
                      } @else if (creditPercent(selectedClient()) >= 80) {
                        <div class="banner banner-warning" style="margin-top:8px"><i class="pi pi-exclamation-triangle"></i><div>{{ t('orders.new.creditWarning', { pct: creditPercent(selectedClient()) }) }}</div></div>
                      }
                    } @else {
                      <div style="font-size:12px;color:#6B7280;display:flex;align-items:center;gap:5px">
                        <i class="pi pi-credit-card"></i> {{ t('orders.new.cashClient') }}
                      </div>
                    }
                  </div>
                  <button class="btn btn-primary" style="width:100%;justify-content:center" [disabled]="isCreditBlocked()" (click)="proceedToProducts()">
                    {{ t('common.next') }} <i class="pi pi-arrow-right"></i>
                  </button>
                }
              </div>
            </div>
        ...
        @if (step() === 2) {
          <div>
            <div class="card-title" style="margin-bottom:12px">{{ t('orders.new.addProductsFor', { name: selectedClient()?.name || '' }) }}</div>
            <div class="grid-2" style="align-items:start">
              <div class="card" style="overflow:hidden">
                <div class="card-header"><span class="card-title">{{ t('nav.catalog') }}</span></div>
                <table class="data-table">
                  <thead><tr><th>{{ t('orderDetail.product') }}</th><th>{{ t('orders.new.available') }}</th><th>{{ t('orders.new.price') }}</th><th></th></tr></thead>
                  <tbody>
              ...
              <div class="card card-pad" style="position:sticky;top:24px">
                <div class="card-title" style="margin-bottom:12px">{{ t('orders.new.summary', { count: lines().length }) }}</div>
                @if (!lines().length) {
                  <div class="empty-state" style="padding:24px">
                    <div class="empty-state-icon"><i class="pi pi-shopping-cart"></i></div>
                    <div class="empty-state-title">{{ t('orders.new.emptyProductsTitle') }}</div>
                    <div class="empty-state-desc">{{ t('orders.new.emptyProductsDesc') }}</div>
                  </div>
                } @else {
                  @for (l of lines(); track l.productId; let i = $index) {
                    <div style="display:flex;gap:8px;align-items:center;padding:8px 0;border-bottom:1px solid #F3F0EC">
                      <div style="flex:1">
                        ...
                        <div style="font-size:11px;color:#9CA3AF">S/ {{ l.price | number:'1.2-2' }} / {{ l.unit }} · {{ t('orders.new.max') }} {{ l.max }}</div>
                      </div>
                  ...
                  <div style="display:flex;justify-content:space-between;font-size:13px;margin-top:12px">
                    <span style="color:#6B7280">{{ t('orderDetail.subtotal') }}</span><span>S/ {{ subtotal() | number:'1.2-2' }}</span>
                  </div>
                  <div style="display:flex;justify-content:space-between;font-weight:700;font-size:15px;margin-top:8px;padding-top:12px;border-top:2px solid #E5E7EB">
                    <span>{{ t('orderDetail.total') }}</span><span>S/ {{ total() | number:'1.2-2' }}</span>
                  </div>
                  @if (hasInvalidLines()) {
                    <div class="banner banner-danger" style="margin-top:12px">
                      <i class="pi pi-exclamation-triangle"></i>
                      <div>{{ t('orders.new.invalidQuantities') }}</div>
                    </div>
                  }
        ...
        @if (step() === 3) {
          <div class="card card-pad" style="max-width:560px">
            <div class="card-title" style="margin-bottom:16px">{{ t('orders.new.deliveryInfo') }}</div>
            <div class="field" style="margin-bottom:14px">
              <div class="field-label">{{ t('orders.new.deliveryDate') }}</div>
              <div class="field-input"><i class="pi pi-calendar"></i><input type="date" [(ngModel)]="deliveryDate" /></div>
            </div>
            <div class="field" style="margin-bottom:14px">
              <div class="field-label">{{ t('orders.new.address') }}</div>
              <div class="field-input"><i class="pi pi-map-marker"></i><input type="text" [(ngModel)]="deliveryAddress" /></div>
            </div>
            <div class="field" style="margin-bottom:14px">
              <div class="field-label">{{ t('orders.table.priority') }}</div>
              <div class="field-input"><i class="pi pi-flag"></i>
                <select [(ngModel)]="priority"><option value="low">{{ t('priority.low') }}</option><option value="medium">{{ t('priority.medium') }}</option><option value="high">{{ t('priority.high') }}</option></select>
              </div>
            </div>
            <div class="field" style="margin-bottom:14px">
              <div class="field-label">{{ t('orders.new.notesOptional') }}</div>
              <div class="field-input" style="align-items:flex-start"><i class="pi pi-pencil" style="margin-top:2px"></i>
                <textarea [(ngModel)]="notes" rows="3" style="border:none;outline:none;font-size:13px;flex:1;background:transparent;resize:none" [placeholder]="t('orders.new.notesPlaceholder')"></textarea>
              </div>
            </div>
            <div style="display:flex;gap:8px;margin-top:20px">
              <button class="btn btn-ghost" (click)="step.set(2)"><i class="pi pi-arrow-left"></i> {{ t('common.back') }}</button>
              <button class="btn btn-primary" style="flex:1;justify-content:center" [disabled]="!deliveryDate || !deliveryAddress" (click)="step.set(4)">{{ t('orders.new.continueConfirm') }}</button>
            </div>
          </div>
        }
        ...
        @if (step() === 4) {
          <div class="grid-2" style="align-items:start">
            <div class="card card-pad">
              <div class="card-title" style="margin-bottom:12px">{{ t('orders.table.client') }}</div>
              ...
              <div class="card-title" style="margin-bottom:12px">{{ t('orders.new.items') }}</div>
            ...
            <div class="card card-pad">
              <div class="card-title" style="margin-bottom:12px">{{ t('orders.new.deliveryDetails') }}</div>
              <div style="font-size:13px"><strong>{{ t('orders.table.date') }}:</strong> {{ deliveryDate }}</div>
              <div style="font-size:13px;margin-top:4px"><strong>{{ t('orders.table.priority') }}:</strong> {{ t('priority.' + priority) }}</div>
              @if (notes) { <div style="font-size:13px;margin-top:4px"><strong>{{ t('common.notes') }}:</strong> {{ notes }}</div> }
              <div class="banner banner-info" style="margin-top:16px">
                <i class="pi pi-info-circle"></i>
                <div>{{ t('orders.new.validationNotice') }}</div>
              </div>
              @if (submitError()) {
                <div class="banner banner-danger"><i class="pi pi-times-circle"></i><span>{{ submitError() }}</span></div>
              }
              <div style="display:flex;gap:8px;margin-top:16px">
                <button class="btn btn-ghost" (click)="step.set(3)"><i class="pi pi-arrow-left"></i> {{ t('common.back') }}</button>
                <button class="btn btn-primary" style="flex:1;justify-content:center" (click)="confirm()" [disabled]="submitting() || hasInvalidLines()">
                  @if (submitting()) { <i class="pi pi-spin pi-spinner"></i> {{ t('common.saving') }} } @else { <i class="pi pi-check"></i> {{ t('orders.new.confirmOrder') }} }
                </button>
              </div>
            </div>
          </div>
...
export class OrderNewPage {
  ...
  readonly steps = ['orders.new.steps.client', 'orders.new.steps.products', 'orders.new.steps.delivery', 'orders.new.steps.confirm'];
  ...
  constructor() {
    forkJoin({
      ...
      error: () => {
        this.loadError.set(this.i18n.t('orders.loadUnavailable'));
        this.loading.set(false);
      },
    });
  }
  ...
  addLine(p: Product): void {
    ...
    this.lines.set([...current, { productId: p.id, backendProductId: p.backendId, qty: 1, price: p.price, name: p.name, unit: p.unit, max }]);
  }
  ...
  confirm(): void {
    ...
    const items: OrderItem[] = this.lines().map((l) => ({
      ...
      backendProductId: l.backendProductId,
      qty: l.qty,
      price: l.price,
      stockOk: l.qty <= l.max,
    }));
    ...
    this.api.create(payload).subscribe({
      ...
      error: (e) => {
        ...
        if (code === 404 || code === 405 || code === 0) {
          this.submitError.set(this.i18n.t('orders.createUnavailable'));
        } else {
          this.submitError.set(this.i18n.t('orders.new.createHttpError', { code }));
        }
      },
    });
  }
