...
const isLogistics = computed(() => roleKey.value === 'logistics' || roleKey.value === 'admin');
...
const inTransitCount = computed(() => D.dispatches.filter(d => d.status === 'in_transit').length);
const pendingEvidenceCount = computed(() => D.dispatches.filter(d => d.evidenceRequired && !d.evidenceDone).length);

function handleAlert(a) {
  if (a.screen === 'inventory') router.push('/ops/inventory');
  else if (a.screen === 'dispatch') router.push('/ops/dispatch');
  else if (a.screen === 'orders') router.push('/ops/orders');
  else if (a.screen === 'clients') router.push('/ops/clients');
}
    ...
    <div>
      <div class="card" style="overflow:hidden;margin-bottom:12px">
        <div class="card-header">
          ...
          <button class="btn btn-ghost btn-sm" @click="router.push('/ops/orders')">{{ t('dashboard.viewAll') }}</button>
        </div>
        <table class="data-table" role="table" :aria-label="t('dashboard.reqAction')">
          ...
          <tbody>
            <tr v-for="o in pendingOrders" :key="o.id" style="cursor:pointer" @click="router.push(`/ops/orders/${o.id}`)">
      ...
      <div class="card card-pad">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          <template v-if="isLogistics">
            <button class="btn btn-ghost" style="flex-direction:column;padding:14px 8px;gap:6px;font-size:11px;height:auto" @click="router.push('/ops/inventory')"><i class="pi pi-database" style="font-size:20px" aria-hidden="true"></i>{{ t('nav.inventory') }}</button>
            <button class="btn btn-ghost" style="flex-direction:column;padding:14px 8px;gap:6px;font-size:11px;height:auto" @click="router.push('/ops/dispatch')"><i class="pi pi-send" style="font-size:20px" aria-hidden="true"></i>{{ t('nav.dispatch') }}</button>
            <button class="btn btn-ghost" style="flex-direction:column;padding:14px 8px;gap:6px;font-size:11px;height:auto" @click="router.push('/ops/reports')"><i class="pi pi-chart-bar" style="font-size:20px" aria-hidden="true"></i>{{ t('nav.reports') }}</button>
            <button class="btn btn-ghost" style="flex-direction:column;padding:14px 8px;gap:6px;font-size:11px;height:auto" @click="router.push('/ops/orders/new')"><i class="pi pi-plus-circle" style="font-size:20px" aria-hidden="true"></i>{{ t('nav.createOrder') }}</button>
          </template>
          <template v-else>
            <button class="btn btn-ghost" style="flex-direction:column;padding:14px 8px;gap:6px;font-size:11px;height:auto" @click="router.push('/ops/clients')"><i class="pi pi-users" style="font-size:20px" aria-hidden="true"></i>{{ t('nav.clients') }}</button>
            <button class="btn btn-ghost" style="flex-direction:column;padding:14px 8px;gap:6px;font-size:11px;height:auto" @click="router.push('/ops/orders/new')"><i class="pi pi-plus-circle" style="font-size:20px" aria-hidden="true"></i>{{ t('nav.createOrder') }}</button>
            <button class="btn btn-ghost" style="flex-direction:column;padding:14px 8px;gap:6px;font-size:11px;height:auto" @click="router.push('/ops/orders')"><i class="pi pi-file-edit" style="font-size:20px" aria-hidden="true"></i>{{ t('nav.orders') }}</button>
            <button class="btn btn-ghost" style="flex-direction:column;padding:14px 8px;gap:6px;font-size:11px;height:auto" @click="router.push('/ops/catalog')"><i class="pi pi-box" style="font-size:20px" aria-hidden="true"></i>{{ t('nav.catalog') }}</button>
          </template>
        </div>
      </div>
