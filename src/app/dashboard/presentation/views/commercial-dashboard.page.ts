...
import { ActivityEntry } from '@app/dashboard/domain/model';
    ...
    <div class="page">
      ...
      <div class="flow-grid-12">
            ...
            @for (request of newRequests().slice(0, 5); track request.id) {
              <div class="flow-list-item">
                <div>
                  ...
                  <div class="flow-note">{{ request.notes || 'Validar disponibilidad, crédito y ventana de entrega.' }}</div>
                </div>
        ...
        <section class="flow-panel span-4">
          <div class="flow-panel-head">
            <div>
              <div class="flow-title">{{ 'commercialDashboard.creditRequests' | t }}</div>
              <div class="flow-subtitle">{{ 'commercialDashboard.creditRequestsSub' | t }}</div>
            </div>
          </div>
          <div class="flow-panel-pad flow-stack">
            @for (request of pendingCreditRequests().slice(0, 4); track request.id) {
              <div class="flow-list-item">
                <div>
                  <div class="flow-row" style="margin-bottom:5px">
                    <span class="mono">{{ request.id }}</span>
                    <span class="badge badge-amber">{{ 'orders.status.blocked' | t }}</span>
                  </div>
                  <div style="font-size:13px;font-weight:800">{{ clientName(request.clientId) }}</div>
                  <div class="flow-note">{{ requestedAmountLabel(request) }}</div>
                </div>
              </div>
            } @empty {
              <div class="flow-note">{{ 'commercialDashboard.noCreditRequests' | t }}</div>
            }
          </div>
        </section>

        <section class="flow-panel span-4">
          <div class="flow-panel-head"><div class="flow-title">{{ 'commercialDashboard.quickActions' | t }}</div></div>
          <div class="flow-panel-pad flow-stack">
            <button class="btn btn-primary" (click)="go('/orders/new')"><i class="pi pi-plus" aria-hidden="true"></i> {{ 'commercialDashboard.manualOrder' | t }}</button>
            <button class="btn btn-secondary" (click)="go('/commercial/purchase-requests')"><i class="pi pi-inbox" aria-hidden="true"></i> {{ 'commercialDashboard.requestsButton' | t }}</button>
            <button class="btn btn-secondary" (click)="go('/orders')"><i class="pi pi-file-edit" aria-hidden="true"></i> {{ 'nav.orders' | t }}</button>
            <button class="btn btn-secondary" (click)="go('/clients')"><i class="pi pi-users" aria-hidden="true"></i> {{ 'nav.clients' | t }}</button>
            <button class="btn btn-secondary" (click)="go('/commercial/business-documents')"><i class="pi pi-file-check" aria-hidden="true"></i> {{ 'commercialDashboard.businessDocuments' | t }}</button>
          </div>
        </section>

        <section class="flow-panel span-8">
          <div class="flow-panel-head">
            <div class="flow-title">{{ 'commercialDashboard.recentActivity' | t }}</div>
            <span class="status-label">{{ 'commercialDashboard.operationalActivity' | t }}</span>
          </div>
          <div class="flow-panel-pad">
            @for (item of activity().slice(0, 7); track item.id) {
              <div class="activity-item">
                <div class="activity-dot" [style.background]="activityColor(item.type)" aria-hidden="true"></div>
...
export class CommercialDashboardPage {
  ...
  readonly pendingDocs = computed(() =>
    this.dispatches().filter((d) => d.evidenceRequired && !d.evidenceDone)
  );
  ...
  constructor() {
    forkJoin({
      ...
      next: ({ activity, orders, dispatches, clients }) => {
