      ...
      <div class="flow-grid-12">
            ...
            @for (dispatch of dispatches().slice(0, 5); track dispatch.id) {
              <div class="flow-list-item">
                <div>
                  ...
                  <div style="font-size:13px;font-weight:800">{{ clientName(dispatch.clientId) }}</div>
                  <div class="flow-note">
        ...
        <section class="flow-panel span-12 operations-quick-actions">
          ...
          <div class="flow-panel-pad quick-action-grid">
            <button class="btn btn-secondary" (click)="go('/inventory')"><i class="pi pi-database" aria-hidden="true"></i> Inventory Control</button>
            <button class="btn btn-primary" (click)="go('/dispatches')"><i class="pi pi-send" aria-hidden="true"></i> Dispatch Orders</button>
            <button class="btn btn-secondary" (click)="go('/proof-of-delivery')"><i class="pi pi-camera" aria-hidden="true"></i> Proof of Delivery</button>
            <button class="btn btn-secondary" (click)="go('/analytics')"><i class="pi pi-chart-bar" aria-hidden="true"></i> Operational Analytics</button>
            <button class="btn btn-secondary" (click)="go('/operations/business-documents')"><i class="pi pi-file-check" aria-hidden="true"></i> Business Documents</button>
            <button class="btn btn-secondary" (click)="go('/operations/promotions')"><i class="pi pi-megaphone" aria-hidden="true"></i> Promotions</button>
            <button class="btn btn-secondary" (click)="go('/customer-portals')"><i class="pi pi-upload" aria-hidden="true"></i> Customer Portals</button>
            <button class="btn btn-secondary" (click)="go('/operations/company-administration')"><i class="pi pi-building" aria-hidden="true"></i> Company Administration</button>
          </div>
        </section>
      </div>
...
export class OperationsDashboardPage {
  ...
  constructor() {
    forkJoin({
      ...
      next: ({ products, dispatches, clients, lots }) => {
  ...
  creditFor(dispatch: Dispatch) {
    const client = this.clients().find((c) => c.id === dispatch.clientId);
  ...
  displayCode(dispatch: Dispatch): string {
    return dispatch.code || dispatch.id || '';
  }
  ...
  coldTypeLabel(type?: string): string {
    ...
    return map[type || 'chilled'] ?? (type || 'Chilled');
  }
