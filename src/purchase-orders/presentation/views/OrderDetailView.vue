...
function cancelOrder() {
  ...
  setTimeout(() => router.push('/ops/orders'), 1500);
}
...
const timeline = computed(() => {
  ...
  return ORDER_STATUS_FLOW.map((status, index) => {
    ...
    return {
      ...
      meta: index === 0 ? order.value.date : state === 'active' ? t('common.view') : '—',
...
<template>
  <div v-if="!order" class="empty-state" style="padding:64px 20px;max-width:560px;margin:0 auto">
    ...
    <button class="btn btn-primary" style="margin-top:16px" @click="router.push('/ops/orders')">
  ...
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
    <button class="btn btn-ghost btn-sm" @click="router.push('/ops/orders')"><i class="pi pi-arrow-left"></i> {{ t('nav.orders') }}</button>
    <div style="flex:1">
      ...
      <div class="page-subtitle">{{ ds.clientName(order.clientId) }} · {{ order.date }}</div>
    </div>
    <button class="btn btn-ghost" @click="toast.add({ severity:'info', summary:'Imprimiendo…', detail:'Abre el diálogo de impresión del navegador', life:2500 })"><i class="pi pi-print"></i> Imprimir</button>
    <button class="btn btn-primary" v-if="order.status === 'validating'" @click="confirmOrder"><i class="pi pi-check"></i> Confirmar</button>
    <button class="btn btn-danger" v-if="['blocked','validating'].includes(order.status)" @click="cancelOrder"><i class="pi pi-times"></i> Cancelar</button>
  </div>
  ...
  <div style="display:grid;grid-template-columns:1fr 320px;gap:16px;align-items:start">
    <div>
      <div class="card" style="overflow:hidden;margin-bottom:16px">
        <div class="card-header"><span class="card-title">Productos solicitados</span></div>
        <table class="data-table">
          <thead><tr><th>Producto</th><th>Cantidad</th><th>Precio</th><th>Subtotal</th><th>Stock</th></tr></thead>
          <tbody>
            <tr v-for="i in order.items" :key="i.productId">
              <td>
              ...
              <td>
                <span :class="'badge ' + (i.stockOk ? 'badge-green' : 'badge-red')">
                  {{ i.stockOk ? 'Disponible' : 'Stock parcial' }}
                </span>
              </td>
            </tr>
            <tr v-if="!order.items.length"><td colspan="5"><div class="empty-state" style="padding:24px"><div class="empty-state-icon"><i class="pi pi-box"></i></div><div class="empty-state-title">Sin items</div></div></td></tr>
          </tbody>
        </table>
      ...
      <div class="card card-pad">
        <div class="card-title" style="margin-bottom:16px">Línea de tiempo</div>
        <div class="timeline">
    ...
    <div class="card card-pad" v-if="client">
      <div v-if="order.createdByName" style="margin-bottom:14px;padding:10px 12px;background:#F0F7FF;border:1px solid #BFDBFE;border-radius:8px">
        <div style="font-size:11px;font-weight:600;color:#2563EB;text-transform:uppercase;letter-spacing:.04em;margin-bottom:4px">Registrado por</div>
      ...
      <div class="divider" style="margin:14px 0"></div>
      <div style="font-size:12px;color:#6B7280">Condición</div>
      <div style="font-size:13px">{{ client.condition }}</div>
      <template v-if="client.creditLimit">
        <div class="divider" style="margin:14px 0"></div>
        <div style="font-size:12px;color:#6B7280;margin-bottom:6px">Crédito utilizado</div>
