...
<template>
  ...
  >
    <div class="drawer-header">
      <div class="drawer-title">Perfil del cliente</div>
      <button class="btn btn-ghost btn-sm" @click="closeClient" aria-label="Cerrar"><i class="pi pi-times"></i></button>
    </div>
    <div class="drawer-body" v-if="detail">
      ...
      <div class="card card-pad" style="margin-bottom:12px">
        <div style="font-size:10px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px">Contacto</div>
      ...
      <!-- Commercial -->
      <div class="card card-pad" style="margin-bottom:12px">
        <div style="font-size:10px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px">Condiciones comerciales</div>
        <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:8px">
          <span style="color:#6B7280">Condición de pago</span>
          <span style="font-weight:600">{{ detail.condition }}</span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:8px">
          <span style="color:#6B7280">Último pedido</span>
          <span style="font-weight:500">{{ detail.lastOrder }}</span>
        </div>
        <template v-if="detail.creditLimit">
          <div class="divider" style="margin:10px 0"></div>
          <div style="display:flex;justify-content:space-between;font-size:11px;color:#6B7280;margin-bottom:4px">
            <span>Utilizado: S/ {{ detail.creditUsed.toLocaleString() }}</span>
            <span>Límite: S/ {{ detail.creditLimit.toLocaleString() }}</span>
          </div>
          ...
          <div style="text-align:right;font-size:11px;color:#6B7280">{{ creditPercent(detail) }}% utilizado</div>
          <div v-if="detail.creditUsed >= detail.creditLimit" class="banner banner-danger" style="margin-top:8px">
            <i class="pi pi-times-circle"></i>
            <div>Crédito agotado. Pedidos quedarán bloqueados.</div>
          </div>
        </template>
        <template v-else>
          <div class="divider" style="margin:10px 0"></div>
          <div style="font-size:12px;color:#6B7280"><i class="pi pi-credit-card"></i> Cliente al contado</div>
        </template>
      </div>
