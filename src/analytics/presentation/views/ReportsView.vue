...
const activeDispatches = computed(() => D.dispatches.filter(d => d.status === 'in_transit').length);
const pendingEvidence  = computed(() => D.dispatches.filter(d => d.evidenceRequired && !d.evidenceDone).length);
...
<template>
  <div class="page-header" role="banner">
    <div>
      <div class="page-title">{{ isLogistics ? t('reports.titleLogistics') : t('reports.titleCommercial') }}</div>
      <div class="page-subtitle">
        {{ D.company.name }} · {{ isLogistics ? t('reports.subtitleLogistics') : t('reports.subtitleCommercial') }}
      </div>
    </div>
  </div>
  ...
  <div class="grid-4" style="margin-bottom:20px">
    <div class="card kpi-card">
      <div class="kpi-label"><i class="pi pi-file-edit" style="color:#2563EB"></i> {{ t('reports.activeOrders') }}</div>
      <div class="kpi-value" style="color:#2563EB">{{ activeOrders }}</div>
      <div class="kpi-sub">{{ t('reports.ofPeriod', { total: totalOrders }) }}</div>
    </div>
    <div class="card kpi-card">
      <div class="kpi-label"><i class="pi pi-check-circle" style="color:#22C55E"></i> {{ t('reports.completedDeliveries') }}</div>
      <div class="kpi-value" style="color:#22C55E">{{ deliveredOrders }}</div>
      <div class="kpi-sub">{{ t('reports.billed', { amount: totalRevenue.toFixed(2) }) }}</div>
    </div>
    <div v-if="isLogistics" class="card kpi-card">
      <div class="kpi-label"><i class="pi pi-send" style="color:#F59E0B"></i> {{ t('reports.inTransit') }}</div>
      <div class="kpi-value" style="color:#F59E0B">{{ activeDispatches }}</div>
      <div class="kpi-sub">{{ t('reports.withoutEvidence', { n: pendingEvidence }) }}</div>
    </div>
    <div v-else class="card kpi-card">
      <div class="kpi-label"><i class="pi pi-wallet" style="color:#F59E0B"></i> {{ t('reports.pendingCollection') }}</div>
      <div class="kpi-value" style="color:#F59E0B">S/ {{ pendingRevenue.toFixed(0) }}</div>
      <div class="kpi-sub">{{ t('reports.inPrepDispatch') }}</div>
    </div>
    <div class="card kpi-card">
      <div class="kpi-label"><i class="pi pi-ban" style="color:#EF4444"></i> {{ t('reports.blocked') }}</div>
      <div class="kpi-value" style="color:#EF4444">{{ blockedOrders }}</div>
      <div class="kpi-sub">{{ t('reports.requireAction') }}</div>
    </div>
  </div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;align-items:start;margin-bottom:20px">
    ...
    <div class="card" style="overflow:hidden">
      <div class="card-header"><span class="card-title">{{ t('reports.byStatus') }}</span></div>
      <table class="data-table">
        <thead><tr><th>{{ t('reports.table.status') }}</th><th>{{ t('reports.table.count') }}</th><th>{{ t('reports.table.pct') }}</th></tr></thead>
        <tbody>
    ...
    <div class="card card-pad">
      <div class="card-title" style="margin-bottom:16px">{{ t('reports.byCategory') }}</div>
  ...
  <div v-if="isLogistics" style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
    ...
    <div class="card" style="overflow:hidden">
      <div class="card-header">
        <span class="card-title">{{ t('reports.expiringLots') }}</span>
        <span style="font-size:12px;color:#6B7280">{{ t('reports.lotsDays') }}</span>
      </div>
      <div v-if="!expiringLots.length" style="padding:24px;text-align:center;color:#9CA3AF;font-size:13px">
        {{ t('reports.noExpiring') }}
      </div>
      <table v-else class="data-table">
        <thead><tr><th>{{ t('reports.table.lot') }}</th><th>{{ t('reports.table.product') }}</th><th>{{ t('reports.table.expiry') }}</th><th>{{ t('reports.table.available') }}</th></tr></thead>
        <tbody>
    ...
    <div class="card" style="overflow:hidden">
      <div class="card-header">
        <span class="card-title">{{ t('reports.criticalStock') }}</span>
        <span style="font-size:12px;color:#6B7280">{{ t('reports.criticalStockSub') }}</span>
      </div>
      <div v-if="!lowStock.length" style="padding:24px;text-align:center;color:#9CA3AF;font-size:13px">
        {{ t('reports.allStockNormal') }}
      </div>
      <table v-else class="data-table">
        <thead><tr><th>{{ t('reports.table.product') }}</th><th>{{ t('reports.table.available') }}</th><th>{{ t('reports.table.minimum') }}</th><th>{{ t('reports.table.status') }}</th></tr></thead>
        <tbody>
          <tr v-for="p in lowStock" :key="p.id">
            ...
            <td>
              <span :class="'badge ' + (p.status === 'out' ? 'badge-red' : 'badge-amber')">
                {{ p.status === 'out' ? t('reports.outOfStock') : t('reports.lowStock') }}
              </span>
            </td>
          </tr>
        </tbody>
