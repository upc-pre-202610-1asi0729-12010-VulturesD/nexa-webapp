...
const currentTimeLabel = computed(() => new Intl.DateTimeFormat('es-PE', { hour: '2-digit', minute: '2-digit' }).format(new Date()));
...
<template>
  ...
  <div class="banner banner-danger" v-if="expiringLots.length" role="alert">
    ...
    <div><strong>{{ expiringLots.length }} lote(s) vencen pronto</strong> —
      {{ expiringLots.map(l => ds.productName(l.productId) + ' (' + l.id + ')').join(' · ') }}.
      Prioriza su salida según FEFO.
    </div>
  </div>
  ...
  <div v-if="tab === 'lots'" role="tabpanel">
    <div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:12px;padding:16px;margin-bottom:16px">
      ...
      <div style="font-size:14px;font-weight:600;color:#1E3A8A;margin-bottom:5px" v-if="urgentLot">
        Prioriza la salida de {{ ds.productName(urgentLot.productId) }}
      </div>
      <div style="font-size:12px;color:#3B4E6B;line-height:1.5" v-if="urgentLot">
        Lote <strong>{{ urgentLot.id }}</strong> vence el <strong>{{ urgentLot.expiry }}</strong>. Quedan {{ urgentLot.qty - urgentLot.reserved }} {{ ds.productById(urgentLot.productId)?.unit }} disponibles.
      </div>
    </div>
    <div class="card" style="overflow:hidden">
      <div class="card-header">
      ...
      <table class="data-table" role="table" :aria-label="t('inventory.lotsByExpiry')">
        ...
        <tbody>
          ...
          >
            ...
            <td>
              <span :class="daysUntil(lot.expiry) <= 7 ? 'badge badge-red' : daysUntil(lot.expiry) <= 30 ? 'badge badge-amber' : 'badge badge-green'">{{ daysUntil(lot.expiry) }} días</span>
            </td>
  ...
  <div v-if="tab === 'movements'" class="card" style="overflow:hidden" role="tabpanel">
    <div class="card-header">
      ...
      <button class="btn btn-secondary btn-sm" disabled title="Disponible en AV2">
  ...
  <aside
    ...
    role="dialog"
    aria-modal="true"
    aria-label="Detalle de lote"
  >
    <div class="drawer-header">
      <div class="drawer-title">
        <span class="mono">{{ lotDrawer.lot?.id }}</span>
      </div>
      <button class="btn btn-ghost btn-sm" @click="closeLot" aria-label="Cerrar">
    ...
    <div class="drawer-body" v-if="lotDrawer.lot">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
        <div style="background:#F9FAFB;border-radius:8px;padding:10px">
          <div style="font-size:10px;font-weight:600;color:#6B7280;text-transform:uppercase;margin-bottom:4px">Ingreso</div>
          <div style="font-size:13px;font-weight:500">{{ lotDrawer.lot.entryDate }}</div>
        </div>
        <div style="background:#F9FAFB;border-radius:8px;padding:10px">
          <div style="font-size:10px;font-weight:600;color:#6B7280;text-transform:uppercase;margin-bottom:4px">Vencimiento</div>
          <div style="font-size:13px;font-weight:500" :style="{ color: daysUntil(lotDrawer.lot.expiry) <= 7 ? '#B91C1C' : daysUntil(lotDrawer.lot.expiry) <= 30 ? '#B45309' : '#111827' }">
            ...
            <span style="font-size:11px;margin-left:4px">({{ daysUntil(lotDrawer.lot.expiry) }} días)</span>
          </div>
        </div>
        <div style="background:#F9FAFB;border-radius:8px;padding:10px">
        ...
        <div style="background:#F9FAFB;border-radius:8px;padding:10px">
          <div style="font-size:10px;font-weight:600;color:#6B7280;text-transform:uppercase;margin-bottom:4px">Disponible</div>
      ...
      <div v-if="!lotMovements.length" style="font-size:12px;color:#9CA3AF;text-align:center;padding:20px 0">
        Sin movimientos para este lote
      </div>
